...
export default function OrderTable({ orders }) {
  const [selectedOrder, setSelectedOrder] = useState(null);
  const [orderList, setOrderList] = useState(orders); // Local state for delete update
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`https://namaexpressbackend.onrender.com/api/orders/${id}`, {
        method: "DELETE",
      });
      ...
      <div className="p-4">
        <div className="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50">
              <tr>
                {["SN", "Order ID", "Items", "Total", "Status", "Date", "Action"].map(
                  (heading) => (
                    <th
                      key={heading}
                      className="px-6 py-3 font-medium text-gray-700 tracking-wider"
